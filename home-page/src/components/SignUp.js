...
const SignUp = () => {
    ...
    return (
        <>
            ...
            <div className={' absolute left-[32%] bottom-[35px] bg-[ghostwhite] rounded-md border-[2px] border-solid shadow-2xl h-[83vh] w-[32%]'} >
                <h1 className="text-center text-[30px] h-[125px] pt-[50px] font-sans font-extrabold">Sign Up</h1>
                <form onSubmit={handleSubmit} className="flex flex-col justify-evenly" id="email-form" name="email-form" data-name="Email Form">
                    ...
                    <div className="self-center">
                        <label className="font-serif leading-[-1px] uppercase font-bold" htmlFor="username">Username</label>
                    </div>
                    <div className="w-full">
                        <input id="username" type="username" placeholder="Enter your username"
                            name="username" data-name="username" required="required"
                            value={values.username}
                            onChange={handleChange}
                            onBlur={handleBlur}
                            className={`block w-[50%] lg:w-[90%] h-[38px] py-2 px-3 mb-[10px] leading-[1.42857143] text-[#555] bg-[#fff] focus:border-cyan-400 outline-none border-[1px] border-solid  m-auto font-serif text-xs 
                    ${touched.username && errors.username ? "border-red-600 " : "border-[#ccc]"}`}
                        />
                    </div>
                    {touched.username && errors.username ? <p className='text-center text-red-600 pb-[1.5rem]'>{errors.username}</p> : null}
                    <div className="self-center">
                    ...
                    <div className="self-center">
                        <button className="iinline-block px-6 py-2.5 text-white font-medium text-xs leading-tight uppercase rounded bg-slate-700 shadow-md hover:bg-blue-700 hover:shadow-lg focus:shadow-lg focus:outline-none focus:ring-0 active:shadow-lg transition-all duration-200 ease-linear w-full mt-4 " type="submit" value="Submit"
                            data-wait="Please wait..." disabled={isSubmitting}>Sign Up</button>
                    </div>
                </form>
                <h1 className="text-center text-black h-[100px] pt-[30px] font-bold font-serif">Already Signed UP?</h1>
                <div className='text-center'>
                    <Link to='/login' className=' text-black my-[5px] font-sans text-[30px] xl:text-[32px] md:text-[28px] md:leading-[27px] xl:leading-[31px] leading-10 font-bold tracking-[-3px] hover:text-blue-700 hover:underline decoration-2 decoration-black transition-all duration-200 ease-linear'>Log In</Link>
                </div>
